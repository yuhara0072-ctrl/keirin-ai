...
from db import ensure_db, get_db_counts_fast, get_db_status, init_db
...
def get_cached_dashboard_status(*, force: bool = False) -> dict:
    if not force and DB_COUNTS_SESSION_KEY in st.session_state:
        return dict(st.session_state[DB_COUNTS_SESSION_KEY])
    status = get_db_counts_fast()
    status["source"] = "db" if status.get("races", 0) > 0 else "empty"
    st.session_state[DB_COUNTS_SESSION_KEY] = status
    return status
...
def get_quick_data_status() -> dict:
    return get_cached_dashboard_status()


def prompt_load_bundles_if_needed(tab_label: str, bet_type: str, tab_key: str) -> bool:
    return prompt_load_tab(tab_label, tab_key, bet_type)
...
with safe_page("サイドバー"):
    with st.sidebar:
        ...
        if run_btn:
            ...
            invalidate_db_counts_cache()
            get_cached_dashboard_status(force=True)
...
if _check_deep:
    load_tab_bundle("check", bet_type, deep_check=True)
...
_dashboard_status = get_cached_dashboard_status()
...
data_progress["trust"] = {
    **data_progress.get("trust", {}),
    "label": (data_progress.get("trust") or {}).get("label", "—") + "（タブ読込で更新）",
}
home_dashboard = None
today_todos = [
    {"text": "サイドバー workflow でデータ取得", "done": _dashboard_status["races"] > 0, "tab": "設定"},
    {"text": "🎯 実戦判定で読み込み → 購入判断", "done": False, "tab": "実戦判定"},
]
if _dashboard_status["races"] == 0:
    today_todos.insert(0, {"text": "workflow を実行してデータ取得", "done": False, "tab": "設定"})


MAIN_TABS: list[tuple[str, str]] = [
    ("home", "🏠 ホーム"),
    ("rec", "⭐ 今日のAIおすすめ"),
    ("battle", "🎯 実戦判定"),
    ("market", "📡 市場監視"),
...
_main_keys = [k for k, _ in MAIN_TABS]
if "active_main_tab" not in st.session_state:
    st.session_state.active_main_tab = "home"
_sel_idx = _main_keys.index(st.session_state.active_main_tab) if st.session_state.active_main_tab in _main_keys else 0
active_main = st.selectbox(
    "画面",
    _main_keys,
    index=_sel_idx,
    format_func=lambda k: dict(MAIN_TABS)[k],
    key="main_tab_select",
    label_visibility="collapsed",
)
st.session_state.active_main_tab = active_main

if active_main == "predict":
    _predict_sub = st.radio(
        "予測AI",
        ["predict_ai", "predict_ml", "predict_prerace", "predict_chart"],
        format_func=lambda k: {"predict_ai": "📊 AI指標", "predict_ml": "🤖 ML", "predict_prerace": "⏱ 直前", "predict_chart": "📈 グラフ"}[k],
        horizontal=True, key="predict_sub_tab", label_visibility="collapsed",
    )
else:
    _predict_sub = None

if active_main == "learn":
    _learn_sub = st.radio(
        "学習",
        ["learn", "advanced", "quality", "collect"],
        format_func=lambda k: {"learn": "🧠 パターン", "advanced": "🎓 本格", "quality": "📋 品質", "collect": "📥 収集"}[k],
        horizontal=True, key="learn_sub_tab", label_visibility="collapsed",
    )
else:
    _learn_sub = None

if active_main == "settings":
    _settings_sub = st.radio(
        "設定",
        ["settings_ops", "notify", "analyze", "detect", "report", "help"],
        format_func=lambda k: {"settings_ops": "⚙ 運用", "notify": "🔔 通知", "analyze": "分析", "detect": "異常", "report": "レポート", "help": "使い方"}[k],
        horizontal=True, key="settings_sub_tab", label_visibility="collapsed",
    )
else:
    _settings_sub = None
...
if active_main == "home":
    with safe_page("ホーム"):
        ...
        st.caption("各タブの「読み込む」で詳細表示（初回のみ数秒）")
        st.caption(
            f"DB: レース {_dashboard_status['races']} / 結果 {_dashboard_status['results']} / "
            f"学習 {_dashboard_status.get('learning', 0)}"
        )
        with st.expander("永続化 meta（任意）", expanded=False):
            if st.button("GitHub meta 取得", key="home_meta_btn", use_container_width=True):
                from github_persist import get_persist_meta_summary
                meta = get_persist_meta_summary()
                if meta:
                    st.success(
                        f"races={meta.get('race_count')} results={meta.get('result_count')} "
        ...
        if (data_progress.get("trust") or {}).get("label"):
            st.caption(f"AI信頼度: {data_progress['trust']['label']}")

elif active_main == "rec":
    with safe_page("今日のAIおすすめ"):
        if should_render_tab("今日のAIおすすめ", "rec", bet_type):
            _rec = get_tab_bundle("rec", {})
            recommend_bundle = _rec.get("recommend_bundle", recommend_bundle)
            score_bundle = _rec.get("score_bundle", score_bundle)
...
elif active_main == "battle":
    with safe_page("実戦判定"):
        if should_render_tab("実戦判定", "battle", bet_type):
            battle_bundle = get_tab_bundle("battle", {}).get("battle_bundle", battle_bundle)
            st.subheader("実戦判定")
...
elif active_main == "bankroll":
    with safe_page("資金管理"):
        if should_render_tab("資金管理", "bankroll", bet_type):
            _bk = get_tab_bundle("bankroll", {})
            bankroll_bundle = _bk.get("bankroll_bundle", bankroll_bundle)
            battle_bundle = _bk.get("battle_bundle", battle_bundle)
            st.subheader("資金管理")
            st.caption(
...
elif active_main == "validation":
    with safe_page("検証レポート"):
        if should_render_tab("検証レポート", "validation", bet_type):
            validation_bundle = get_tab_bundle("validation", {}).get(
                "validation_bundle", validation_bundle
            )
            st.subheader("検証レポート")
            ...
            with col_save:
                if st.button("💾 レポート保存", use_container_width=True):
                    path = save_validation_report(
                        bet_type,
                        battle_bundle=get_tab_bundle("battle", {}).get("battle_bundle", battle_bundle),
                        bankroll_plan=get_tab_bundle("bankroll", {}).get("bankroll_bundle", bankroll_bundle),
                    )
...
elif active_main == "improve":
    with safe_page("改善提案"):
        if should_render_tab("改善提案", "improve", bet_type):
            improvement_bundle = get_tab_bundle("improve", {}).get("improvement_bundle", improvement_bundle)
            st.subheader("改善提案AI")
...
elif active_main == "settings" and _settings_sub == "notify":
    with safe_page("通知"):
        st.subheader("通知")
...
elif active_main == "settings" and _settings_sub == "analyze":
    with safe_page("分析"):
        st.subheader("市場偏り分析")
...
elif active_main == "predict" and _predict_sub == "predict_ai":
    with safe_page("AI指標"):
        if should_render_tab("AI指標", "predict_ai", bet_type):
            ai_bundle = get_tab_bundle("predict_ai", {}).get("ai_bundle", ai_bundle)
...
elif active_main == "predict" and _predict_sub == "predict_ml":
    with safe_page("ML予測"):
        if should_render_tab("ML予測", "predict_ml", bet_type):
            ml_bundle = get_tab_bundle("predict_ml", {}).get("ml_bundle", ml_bundle)
...
elif active_main == "learn" and _learn_sub == "learn":
    with safe_page("パターン学習"):
        if should_render_tab("パターン学習", "learn", bet_type):
            learning_bundle = get_tab_bundle("learn", {}).get("learning_bundle", learning_bundle)
            st.subheader("学習状況")
...
elif active_main == "learn" and _learn_sub == "advanced":
    with safe_page("本格学習"):
        if should_render_tab("本格学習", "advanced", bet_type):
            advanced_bundle = get_tab_bundle("advanced", {}).get("advanced_bundle", advanced_bundle)
            st.subheader("本格学習")
...
elif active_main == "settings" and _settings_sub == "settings_ops":
    with safe_page("自動運用"):
        if should_render_tab("自動運用", "settings_ops", bet_type):
            ops_status = get_tab_bundle("settings_ops", {}).get("ops_status", ops_status)
...
elif active_main == "learn" and _learn_sub == "collect":
    with safe_page("100レース収集"):
        if should_render_tab("100レース収集", "collect", bet_type):
            collect_bundle = get_tab_bundle("collect", {}).get("collect_bundle", collect_bundle)
...
elif active_main == "learn" and _learn_sub == "quality":
    with safe_page("データ品質"):
        if should_render_tab("データ品質", "quality", bet_type):
            quality_bundle = get_tab_bundle("quality", {}).get("quality_bundle", quality_bundle)
            st.subheader("データ品質")
...
elif active_main == "pnl":
    with safe_page("収支検証"):
        if should_render_tab("収支検証", "pnl", bet_type):
            _pnl = get_tab_bundle("pnl", {})
            pnl_bundle = _pnl.get("pnl_bundle", pnl_bundle)
            recommend_bundle = _pnl.get("recommend_bundle", recommend_bundle)
            st.subheader("収支検証")
...
elif active_main == "line":
    with safe_page("ライン分析"):
        if should_render_tab("ライン分析", "line", bet_type):
            line_bundle = get_tab_bundle("line", {}).get("line_bundle", line_bundle)
            st.subheader("ライン分析")
...
elif active_main == "predict" and _predict_sub == "predict_chart":
    with safe_page("グラフ"):
        if should_render_tab("グラフ", "predict_chart", bet_type):
            _ch = get_tab_bundle("predict_chart", {})
            charts_bundle = _ch.get("charts_bundle", charts_bundle)
...
elif active_main == "market":
    with safe_page("市場監視"):
        if should_render_tab("市場監視", "market", bet_type):
            market_bundle = get_tab_bundle("market", {}).get("market_bundle", market_bundle)
...
elif active_main == "predict" and _predict_sub == "predict_prerace":
    with safe_page("直前分析"):
        if should_render_tab("直前分析", "predict_prerace", bet_type):
            pre_race_bundle = get_tab_bundle("predict_prerace", {}).get("pre_race_bundle", pre_race_bundle)
...
elif active_main == "settings" and _settings_sub == "detect":
    with safe_page("異常検知"):
...
elif active_main == "settings" and _settings_sub == "report":
    with safe_page("レポート"):
...
elif active_main == "backup":
    with safe_page("バックアップ"):
        if should_render_tab("バックアップ", "backup", bet_type):
            backup_bundle = get_tab_bundle("backup", {}).get("backup_bundle", backup_bundle)
            st.subheader("バックアップ")
...
elif active_main == "check":
    with safe_page("システムチェック"):
        if should_render_tab("システムチェック", "check", bet_type):
            system_check_bundle = get_tab_bundle("check", {}).get("system_check_bundle", system_check_bundle)
...
elif active_main == "settings" and _settings_sub == "help":
    with safe_page("使い方"):
        st.markdown(
            """
    ### メニュー構成（14タブ）
